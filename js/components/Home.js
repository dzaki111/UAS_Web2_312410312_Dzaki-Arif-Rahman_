    ...
    methods: {
        loadData() {
            ...
            if (savedData) {
                ...
                this.logs.unshift({ time: new Date().toLocaleTimeString('id-ID'), type: 'info', message: 'Data berhasil dimuat dari penyimpanan lokal.' });
            } else {
                // Seed data jika local storage kosong
                this.products = [
                ...
                this.saveToStorage();
                this.logs.unshift({ time: new Date().toLocaleTimeString('id-ID'), type: 'info', message: 'Data awal telah diinisialisasi ke Local Storage.' });
            }
        },
        saveToStorage() {
            const data = {
                ...
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    <div class="lg:col-span-3 bg-gray-900 border border-gray-800 rounded-xl overflow-hidden shadow-xl flex flex-col justify-between">
                                ...
                                <table class="w-full text-left border-collapse">
                                    ...
                                    <tbody class="divide-y divide-gray-800 text-gray-300">
                                        ...
                                        <tr v-if="filteredProducts.length === 0">
                                            <td colspan="5" class="p-8 text-center text-gray-500 italic">Tidak ada komoditas logistik yang cocok.</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div class="p-4 bg-gray-950/20 border-t border-gray-800 flex justify-between items-center text-[11px] text-gray-400">
                            <span>Menampilkan {{ paginatedProducts.length }} dari {{ filteredProducts.length }} entitas</span>
                    ...
                    <div class="bg-gray-900 border border-gray-800 rounded-xl p-4 flex flex-col justify-between">
                        ...
                        <div class="pt-4 border-t border-gray-800 mt-4 text-[10px] text-gray-500">
                            Status Gateway: <span class="text-emerald-400 font-bold font-mono">LOKAL-STORAGE</span>
                        </div>
                    </div>
                </div>
            </main>

            <footer class="w-full max-w-7xl mx-auto border-t border-gray-850 pt-4 text-center text-[10px] text-gray-500 mt-6">
                &copy; 2026 Sistem Inventaris Gudang. Mode Penyimpanan Lokal.
            </footer>
        </div>
    `
};
